...
In Python 3, the built-in function `range` returns an iterable **range object** instead of a list. The behavior of `range` is the same as the `xrange` in Python 2. Therefore, using `range` do not take up huge memory anymore if we want to run a code block many times within a loop. Further information can be found on PEP [3100](https://www.python.org/dev/peps/pep-3100).
